    ...
    >
        ...
        <div class="w-360px px-10px pt-30px">
            <div v-show="activePage === 'login'" class="login">
                ...
                <div
                    class="w-full flex flex-col mt-20px gap-20px input_section"
                >
                ...
                <div class="next px-40px py-10px">
                    <button class="link_btn" @click="activePage = 'forget'">
                        忘記密碼?
                    </button>
                    <span>|</span>
                    <button class="link_btn" @click="activePage = 'register'">
                        註冊會員
                    </button>
                </div>
            </div>
            <Register
                ...
                @redirect-register-success="activePage = 'registerSuccess'"
                @close-modal="emit('confirm')"
            />
            <Forget
                v-show="activePage === 'forget'"
                @reset-password="handleForget"
            />
            <AfterSuccess
                :content="`${resetEmail}請到信箱取件重設密碼`"
                v-show="activePage === 'afterSuccess'"
                @redirect-login="activePage = 'login'"
            />
            <AfterSuccess
                content="註冊成功"
                close="true"
                v-show="activePage === 'registerSuccess'"
                @redirect-login="activePage = 'login'"
                @close-modal="emit('confirm')"
            />
            <AfterSuccess
                content="登入成功"
                close="true"
                v-show="activePage === 'loginSuccess'"
                @redirect-login="activePage = 'login'"
                @close-modal="emit('confirm')"
            />
        </div>
